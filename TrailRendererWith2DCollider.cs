...
public class TrailRendererWith2DCollider : MonoBehaviour
{
    ...
    */
    ...
    [SerializeField] internal Material trailMaterial;                  //the material of the trail.  Changing this during runtime will have no effect.
    [SerializeField] internal float lifeTime = 1.0f;                   //the amount of time in seconds that the trail lasts
    [SerializeField] internal float changeTime = 0.5f;                 //time point when the trail begins changing its width (if widthStart != widthEnd)
    [SerializeField] internal float widthStart = 1.0f;                 //the starting width of the trail
    [SerializeField] internal float widthEnd = 1.0f;                   //the ending width of the trail
    [SerializeField] internal float vertexDistanceMin = 0.10f;         //the minimum distance between the center positions
    [SerializeField] internal Vector3 renderDirection = new Vector3(0, 0, -1); //the direction that the mesh of the trail will be rendered towards
    [SerializeField] internal bool colliderIsTrigger = true;           //determines if the collider is a trigger.  Changing this during runtime will have no effect.
    [SerializeField] internal bool colliderEnabled = true;             //determines if the collider is enabled.  Changing this during runtime will have no effect.
    [SerializeField] internal bool pausing = false;                     //determines if the trail is pausing, i.e. neither creating nor destroying vertices
    ...
    internal void Initiation()
    {
        ...
        vertexDistanceMin = trailRenderer.minVertexDistance;
